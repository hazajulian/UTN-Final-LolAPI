...
const regionSectionSchema = new mongoose.Schema(
  {
    title: { type: String, required: true, trim: true },
    entries: { type: [regionEntrySchema], default: [] },
  },
  { _id: false }
);
...
const regionSchema = new mongoose.Schema(
  {
    regionId: { type: String, required: true, trim: true, index: true },
    locale: {
      type: String,
      required: true,
      enum: ["en_US", "es_MX"],
    ...
    sections: { type: [regionSectionSchema], default: [] },
  },
  { timestamps: true }
);

regionSchema.index({ regionId: 1, locale: 1 }, { unique: true });
